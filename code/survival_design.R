...
# Function to find the design for survival endpoints with specified family-wise error rate and power
finddesign_survival <- function(J, K, ns, delta1, delta0, requiredfwer, requiredpower, groupsizes, treatmentsigmas, events) {
  ...
  for (i in 1:J) {
    for (j in 1:J) {
      ...
      sigma[i, j] <- sqrt(sum(events[i, ]) / sum(events[j, ]))
    }
  }
  ...
  # Check for symmetry using identical()
  if (!identical(sigma, t(sigma))) {
    stop("Covariance matrix is not symmetric")
  }
  
  # Alternatively, use all.equal and check for equality
  symmetry_check <- all.equal(sigma, t(sigma))
  if (!isTRUE(symmetry_check)) {
    stop("Covariance matrix is not symmetric: ", symmetry_check)
  }
  ...
  # Alternatively, adjust to the nearest positive semi-definite matrix
